


<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>

<!DOCTYPE html>
<html>
	<head>
		<title>레시피 검색</title>
		<meta charset="utf-8" />
		<meta name="viewport" content="width=device-width, initial-scale=1, user-scalable=no" />
		<c:set var="cp" value = "${pageContext.request.contextPath}"/>
		<c:set var="img" value="F:\\it\\kgit\\project\\workspace\\app\\src\\main\\webapp\\resources\\images\\userRecipeImage\\" />
		<link rel="stylesheet" href="${cp}/resources/assets/css/main3.css" />
	</head>
	<body class="is-preload homepage">
		<div id="page-wrapper">

			<!-- Header -->
				<div id="header-wrapper">
					<header id="header" class="container">
						
						<!-- Logo -->
							<div id="logo">
								<h1><a href="${cp}/">recipe</a></h1>
								<span>요리를 재밋고 즐겁게~☆</span>
							</div>

						<!-- Nav -->
							<c:if test="${sessionScope.login !=null }">
								<nav id="nav">
									<ul>
										<li class="current">
											<a href="${cp}/recipe/recipeList?page=1">전체 레시피</a>
										</li>
										<li><a href="${cp}/recipe/makeRecipe">레시피 등록</a></li>
										<li><a href="${cp}/member/myPage/myRecipe">마이페이지</a>
										</li>
										<li><a href="${cp}/member/logout">로그아웃</a></li>
										<li><a href="${cp}/customerManage/customerService">고객센터</a></li>
										
									</ul>
								</nav>
							</c:if>
							<c:if test="${sessionScope.login ==null }">
								<nav id="nav">
									<ul>
										<li class="current">
											<a href="${cp}/recipe/recipeList?page=1">전체 레시피</a>
										</li>
										
										<li><a href="${cp}/member/login/loginView">로그인/회원가입</a></li>
										<li><a href="${cp}/customerManage/customerService">고객센터</a></li>
									</ul>
								</nav>
							</c:if>
						
					</header>
				</div>

			<!-- Banner -->

			<!-- Main -->
				<div id="main-wrapper">
					<div class= "container">
						<div class="row">
						<!-- Features -->
						<c:forEach var="dto" items="${recipes}">
						<div class="col-3">

							<!-- Links -->
								<section class="widget links">
									<ul class="style2">
										<li>
										<div>
											<a href="/recipe/recipeInfo/${dto.recipe_no}">
												<img width="200px;" height="200px;" alt="" onerror="this.src='${cp}/resources/images/recipe/image_unknown.png'" src="${cp}/resources/images/userRecipeImage/${dto.recipe_image}">
											</a>
										</div>
										<div>
											<div>
											${dto.recipe_name }
											</div>
											<div>
											작성자 | ${dto.user_id }
											</div>
											<div>
											${dto.recipe_like }
											</div>
										</div>
										</li>
									</ul>
								</section>

						</div>	
						</c:forEach>	
						</div>
						
						<!--페이징처리 -->
						<div class="page">
							<c:if test="${page>1}">
								<a href="/recipe/search?page=${page-1}&search=${search}">&lt;</a>
							</c:if>
							<c:forEach begin="${firstPage}" end="${lastPage}" step="1" var="i">
								<c:choose>
									<c:when test="${page==i}">
										<span>${i}</span>
									</c:when>
									<c:otherwise>
										<a href="/recipe/search?page=${i}&search=${search}">${i}</a>
									</c:otherwise>
								</c:choose>								
							</c:forEach>
							<c:if test="${page<totalPage}">
								<a href="/recipe/search?page=${page+1}&search=${search}">&gt;</a>						
							</c:if>
							<!-- 검색 -->
							<div class="search-recipe">
								<input type="text" id="search" name="search">
								<input type="button" onclick="search();" value="검색">
							</div>
							
						</div>
					</div>
				</div>		
					
					<!-- Footer -->
				<div id="footer-wrapper">
					<footer id="footer" class="container">
						<div class="row">
							<div class="col-12">
								<div id="copyright">
									<ul class="menu">
										<li >&copy;㈜ 레시피 대표이사 : 홍길동 ｜ 사업자등록번호 : 333-11-22222 사업자정보확인 서울특별시 용산구 원효로 00길 0 <br>
											호스팅 제공자 : (주)레시피 ｜ 개인정보책임관리자 : 김길동 ｜ 통신판매업신고 : 용산 제 00000호 <br>

											[방문상담·구매·A/S] 용산점: 서울시 용산구 원효로 00길 ｜ [택배A/S접수] 서울시 용산구 원효로0가 00번지  <br>
											[고객센터] 0000-0000 (팩스번호 02-000-0000)1:1문의하기문자상담 ｜ E-mail : recipe_help@recipe.co.kr</li>
									</ul>
								</div>
							</div>
						</div>
					</footer>
				</div>

			</div>

</body>
<script>let cp = "${pageContext.request.contextPath}"</script>
<script type="text/javascript" src="${cp}/resources/assets/js/recipeList.js"></script>
</html>